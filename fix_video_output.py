...
def create_fixed_video_workflow(duration_seconds: int, resolution: tuple, prompt: str):
    ...
    workflow = {
        ...
        "5": {
            ...
            "_meta": {"title": f"Empty Latent Image ({frames} frames for {duration_seconds}s at {width}x{height})"}
        },
        "6": {
            "inputs": {
                "samples": ["3", 0],
        ...
        "13": {
            ...
            "_meta": {"title": f"Context Options - 24 frame window for {frames} total frames"}
        }
    }
    return workflow
                        ...
                        for node_id, output in outputs.items():
                            if "videos" in output:
                                for video in output["videos"]:
                                    ...
                                    # Check file size
                                    import os
                                    file_path = f"/mnt/1TB-storage/ComfyUI/output/{filename}"
                                    if os.path.exists(file_path):
                                        size_mb = os.path.getsize(file_path) / (1024 * 1024)
                                        logger.info(f"📊 File size: {size_mb:.2f} MB")

                                        # Check if file size is reasonable for video duration
                                        expected_min_size = 1.0  # MB minimum for decent quality
                                        if size_mb < expected_min_size:
                                            logger.warning(f"⚠️  Video file seems too small ({size_mb:.2f} MB)")
                                        else:
                                            logger.info(f"✅ File size looks good ({size_mb:.2f} MB)")
...
def test_video_parameters():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        logger.info(f"   Resolution: {test_case['resolution'][0]}x{test_case['resolution'][1]}")
    ...
    for result in results:
        ...
        logger.info(f"{status}: {result['duration']}s @ {res[0]}x{res[1]} - {result['description']}")
    ...
    if any(r['success'] and r['resolution'] == (1024, 1024) for r in results):
        logger.info("✅ 1024x1024 resolution is working - video output issue is FIXED")
    else:
        logger.info("⚠️  1024x1024 resolution failed - may need VRAM optimization or different settings")

    if any(r['success'] and r['duration'] >= 5 for r in results):
        logger.info("✅ 5+ second videos are working - duration issue is FIXED")
    else:
        logger.info("⚠️  5+ second videos failed - may need shorter context windows or more VRAM")
