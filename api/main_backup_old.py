...
DATABASE_URL = f"postgresql://patrick:{DATABASE_PASSWORD}@localhost/tower_consolidated"
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:3000",
        "http://192.168.50.135:8328",
        "http://192.168.50.135:8080",  # Tower Dashboard
        "http://127.0.0.1:8328",       # Local access
        "http://localhost:8328"        # Localhost access
    ],
...
# Security headers middleware for network access
@app.middleware("http")
async def add_security_headers(request, call_next):
    """Add security headers for network-accessible service"""
    response = await call_next(request)

    # Security headers for production network access
...
ECHO_SERVICE_URL = os.getenv('ECHO_SERVICE_URL', 'http://127.0.0.1:8351')
...
async def guest_status():
    """Get system status and capabilities for guest users"""
    return {
        ...
        "message": "You're viewing in guest mode. Some features require authentication.",
        "available_characters": "Characters with trained LoRAs are marked and available for generation",
        "lora_info": "LoRA status indicates which characters have custom training available"
    }

@app.get("/api/anime/projects", response_model=List[AnimeProjectResponse])
async def get_projects(current_user: dict = Depends(guest_or_auth), db: Session = Depends(get_db)):
    """Get all anime projects (Guest access allowed)"""
    projects = db.query(AnimeProject).all()
    return projects

@app.get("/api/anime/characters")
async def get_characters(current_user: dict = Depends(guest_or_auth)):
    """Get all characters with LoRA training status (Guest access allowed)"""
    # Return demo data with LoRA training status for guest mode testing
    demo_characters = [
        {
            "id": 1,
            "name": "Mei Tanaka",
            "description": "Main character from Tokyo Debt Realism project",
            "project_id": 24,
            "project_name": "Tokyo Debt Realism",
            "lora_status": "trained",
            "lora_name": "mei_working_v1",
            "has_lora": True
        },
        {
            "id": 2,
            "name": "Kai Nakamura",
            "description": "Supporting character - financial advisor",
            "project_id": 24,
            "project_name": "Tokyo Debt Realism",
            "lora_status": "not_trained",
            "lora_name": None,
            "has_lora": False
        },
        {
            ...
            "project_id": 25,
            "project_name": "Cyberpunk Adventure",
            "lora_status": "not_trained",
            "lora_name": None,
            "has_lora": False
        }
    ...
    # Add guest mode indicator for demo
    for character in demo_characters:
        character["demo_mode"] = current_user.get("is_guest", False)

    return demo_characters

@app.post("/api/anime/characters")
async def create_character(data: dict, current_user: dict = Depends(require_user_or_admin), db: Session = Depends(get_db)):
    """Create character (Requires authentication)"""
    from sqlalchemy import text
...
async def create_project(project: AnimeProjectCreate, current_user: dict = Depends(require_user_or_admin), db: Session = Depends(get_db)):
...
async def get_project_scenes(project_id: int, current_user: dict = Depends(guest_or_auth), db: Session = Depends(get_db)):
...
async def get_scenes(project_id: int = None, current_user: dict = Depends(guest_or_auth), db: Session = Depends(get_db)):
...
async def create_scene(data: dict, current_user: dict = Depends(require_auth), db: Session = Depends(get_db)):
    ...
    if not data.get("title"):
        raise HTTPException(status_code=422, detail="Scene title is required")
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Failed to create scene: {str(e)}")
...
async def generate_from_scene(
    ...
    # Use hardcoded profiles since generation_profiles table doesn't exist
    if "tokyo" in scene.project_name.lower() or "mei" in scene.project_name.lower():
        profile_name = "tokyo_debt_realism"
        checkpoint = "realisticVision_v51.safetensors"
        lora = None  # Mei LoRA is corrupted, skip for now
    elif "cyberpunk" in scene.project_name.lower() or "goblin" in scene.project_name.lower():
        profile_name = "cyberpunk_anime"
        checkpoint = "AOM3A1B.safetensors"  # Use anime checkpoint for cyberpunk
        lora = None  # No LoRA for cyberpunk yet
    else:
        profile_name = "default_anime"
        checkpoint = "realisticVision_v51.safetensors"
        lora = None
    ...
    # Update scene status (scenes table only has status column, not output_path or updated_at)
    db.execute(text("""
        UPDATE scenes SET status = 'generated'
        WHERE id::text = :id
    """), {"id": scene_id})
    db.commit()
...
# Include SSOT routes
try:
    from api.routes import ssot_content_management
    app.include_router(ssot_content_management.router, prefix="/api/ssot", tags=["SSOT"])
    logger.info("SSOT routes loaded successfully")
except ImportError as e:
    logger.warning(f"Could not load SSOT routes: {e}")
...
if __name__ == "__main__":
    import uvicorn
    # Bind to all interfaces for network accessibility
