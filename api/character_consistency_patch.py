...
class CharacterConsistencyEngine:
    ...
    def analyze_consistency(self, character_id: int, new_generation_data: Dict[str, Any],
                          db_session) -> ConsistencyAnalysisResult:
        ...
        workflow_score = self._compare_workflows(
            latest_version.comfyui_workflow,
            new_generation_data.get('workflow')
        )
...
def create_character_version(db_session, version_data: CharacterVersionCreate) -> int:
    ...
    result = db_session.execute(
        ...
        """),
        {
            ...
            "comfyui_workflow": json.dumps(version_data.comfyui_workflow) if version_data.comfyui_workflow else None,
            "workflow_template_path": version_data.workflow_template_path,
            "generation_parameters": json.dumps(version_data.generation_parameters) if version_data.generation_parameters else None,
...
def update_production_job_with_consistency_data(db_session, job_id: int,
                                               seed: Optional[int] = None,
