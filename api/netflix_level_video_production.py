...
- LTX Video 2B generation (REPLACES DEPRECATED AnimateDiff)
...
class NetflixLevelVideoProducer:
    ...
    Uses LTX Video 2B for 121-frame, 5+ second sequences
    DEPRECATED: AnimateDiff (limited to 16 frames, 2 seconds)
    """
    ...
    async def create_ltx_video_workflow(
        self,
        prompt: str,
        ...
        """
        Create LTX Video 2B workflow for production-quality video generation
        REPLACES DEPRECATED AnimateDiff - generates 121 frames, 5+ seconds
        """
        width, height = map(int, resolution.split('x'))
        frames = 121  # Fixed optimal frame count for LTX 2B
        seed = int(time.time()) % 2147483647

        # LTX Video 2B workflow - PROVEN WORKING
        workflow = {
            # LTX 2B model loader
            "1": {
                "class_type": "CheckpointLoaderSimple",
                "inputs": {
                    "ckpt_name": "ltx-2/ltxv-2b-0.9.8-distilled.safetensors"
                }
            },

            # Separate T5XXL text encoder for better prompting
            "2": {
        ...
        workflow[video_node] = {
            ...
            "inputs": {
                ...
                "frame_rate": 24,
                "loop_count": 0,
                "filename_prefix": f"ltx_scene_{timestamp}",
        ...
        return {
            "workflow": workflow,
            "expected_output": f"/mnt/1TB-storage/ComfyUI/output/ltx_scene_{timestamp}_00001_.mp4",
            "timestamp": timestamp
        }
    ...
    async def generate_scene_video(
        ...
        try:
            # Create workflow
            workflow_data = await self.create_ltx_video_workflow(
                prompt=prompt,
