...
class CharacterGeneration(Base):
    """Character generation tracking for Character Studio"""
    ...
    # ControlNet
    use_controlnet = Column(Boolean, default=False)
    controlnet_model = Column(String(255))
    pose_reference = Column(String(512))

    # Output
    output_path = Column(String(512), nullable=False)
    comfyui_prompt_id = Column(String(100))
...
class CharacterTurnaround(Base):
    """Character turnaround reference images"""
    __tablename__ = "character_turnarounds"

    id = Column(Integer, primary_key=True)
    character_id = Column(Integer, ForeignKey('character_generations.id', ondelete='CASCADE'))
...
class PoseLibrary(Base):
    """Reusable pose library"""
    __tablename__ = "pose_library"

    id = Column(Integer, primary_key=True)
    pose_name = Column(String(100), unique=True, nullable=False)
    pose_type = Column(String(20), nullable=False)  # openpose, depth, canny
    preview_image = Column(String(512))
...
class AnimationSequence(Base):
    """Animation sequences (walk cycles, etc.) - Phase 3 Updated"""
    __tablename__ = "animation_sequences"

    id = Column(Integer, primary_key=True)
    character_id = Column(Integer, ForeignKey('character_generations.id', ondelete='CASCADE'))
    sequence_type = Column(String(50), nullable=False)
    frame_paths = Column(JSON, nullable=False)  # array of image paths
    ...
    metadata_ = Column("metadata", JSON, default=lambda: {})  # Animation metadata, timing info, etc.
...
class PoseSequence(Base):
    """Custom pose sequences for animations - Phase 3"""
    __tablename__ = "pose_sequences"
    
    id = Column(Integer, primary_key=True)
    sequence_name = Column(String(100), nullable=False)
    animation_type = Column(String(100))  # walk_cycle_2d, idle_breathing, etc.
    pose_paths = Column(JSON, nullable=False)  # Array of pose image/data paths
    fps = Column(Integer, default=24)
