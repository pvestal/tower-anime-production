...
ECHO_BRAIN_DATABASE_URL = "postgresql://patrick:tower_echo_brain_secret_key_2025@192.168.50.135/echo_brain"
...
async def list_characters(echo_db: Session = Depends(get_echo_brain_db)):
    ...
    characters = echo_db.query(CharacterProfile).filter(CharacterProfile.is_active == True).all()
    return characters
...
async def get_character(character_id: int, echo_db: Session = Depends(get_echo_brain_db)):
    ...
    character = echo_db.query(CharacterProfile).filter(
...
async def create_character(
    character: CharacterProfileCreate,
    echo_db: Session = Depends(get_echo_brain_db)
):
    """Create new character in Echo Brain database"""
    # Check if character name already exists
    existing = echo_db.query(CharacterProfile).filter(
        CharacterProfile.character_name == character.character_name
    ).first()
    if existing:
...
async def preview_character_prompt(
    ...
    return {
        ...
        "prompt_parts": {
            ...
            "hair": f"{character.hair_color} {character.hair_style}" if character.hair_color and character.hair_style else None,
