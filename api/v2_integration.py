    ...
    def add_character_attribute(
        ...
        try:
            with conn.cursor() as cur:
                ...
                conn.commit()
                logger.info(
                    f"Added attribute {attribute_type}={attribute_value} for character {character_id}"
                )
        except Exception as e:
            conn.rollback()
        ...
        try:
            ...
            if not project:
                project_id = self.db.create_project(
                    project_name, f"Auto-created for {character_name}"
                )
            else:
...
async def complete_job_with_quality(job_id: int, output_path: str, **quality_metrics):
    ...
    return await v2_integration.update_job_with_quality_metrics(
        job_id, output_path, **quality_metrics
    )
