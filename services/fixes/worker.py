...
class AnimeWorker:
    ...
    async def monitor_comfyui_job(self, job_id: str, comfyui_job_id: str, project_id: str):
            ...
            try:
                ...
                if status == "completed":
                    ...
                    if output_files:
                        ...
                        organized_files = await self.organize_output_files(output_files, project_id)
                        ...
                        await self.broadcast_progress(job_id, 100, "Generation complete!")
                        logger.info(f"✅ Job {job_id} completed in {time.time() - start_time:.1f}s")
                        return
                    else:
                        self.job_queue.fail_job(job_id, "No output files generated")
                        return
                    ...
                    progress = min(30 + int(60 * (elapsed / 120)), 90)  # Estimate 2 minutes
                    self.job_queue.update_job_progress(job_id, progress)
                    await self.broadcast_progress(job_id, progress, f"Generating... ({elapsed:.0f}s)")
        ...
        # Timeout
        self.job_queue.fail_job(job_id, f"Job timed out after {max_wait} seconds")
    ...
    async def organize_output_files(self, files: list, project_id: str) -> list:
        ...
        project_dir = Path(f"/mnt/1TB-storage/anime-projects/{project_id}/{timestamp}")
    ...
    async def broadcast_progress(self, job_id: str, progress: int, message: str):
        ...
        try:
            ...
            self.websocket_redis.publish('anime:job:updates', json.dumps(update))
