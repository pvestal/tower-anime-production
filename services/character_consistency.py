...
# from uuid import UUID  # Using integers for compatibility
import numpy as np
...
try:
    ...
    logger.warning(
        "InsightFace not available - face consistency checking disabled")
...
class CharacterConsistencyService:
    ...
    async def initialize(self):
        ...
        if not INSIGHTFACE_AVAILABLE:
            logger.warning(
                "InsightFace not installed - face features disabled")
            return
        ...
        try:
            self.face_analyzer = FaceAnalysis(
                ...
                providers=[
                    f"{self.device.upper()}ExecutionProvider",
    ...
    async def store_character_embedding(
        self, character_id: UUID, image_path: str, force_recompute: bool = False
    ...
    async def check_consistency(
        self, character_id: UUID, image_path: str, threshold: Optional[float] = None
    ) -> Dict[str, Any]:
        """
                ...
                results.append(
                    {
                        "character_id": str(char_id),
                        "similarity_score": (
                            float(best_similarity) if best_similarity > 0 else 0.0
                        ),
                        "passes_threshold": False,
                        "bbox": None,
                    }
    ...
    async def get_variations(
        self, character_id: UUID, variation_type: str = None
    ) -> List[Dict[str, Any]]:
    ...
    async def apply_variation(
        self, character_id: UUID, variation_id: UUID, base_prompt: str
    ) -> str:
