...
export const useEnhancedAnimeStore = defineStore("enhancedAnime", () => {
  ...
  // Domain-agnostic API base path
  const API_BASE = "/api/anime";

  // Domain-agnostic WebSocket URL
  const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
  const host = window.location.host;
  const WS_URL = `${protocol}//${host}/api/anime/ws`;
  ...
  const generationStats = computed(() => {
    ...
    const pending = generationHistory.value.filter((gen) =>
      ["pending", "running", "processing"].includes(gen.status),
    ).length;
  ...
  function handleWebSocketMessage(data) {
    if (data.type === "progress") {
      ...
      const queueIndex = generationQueue.value.findIndex(
      ...
      addNotification(
        `Generation #${data.job_id} failed: ${data.error}`,
        "error",
      );
    }
  }
  ...
  async function loadProjectCharacters(projectId) {
    try {
      const response = await fetch(
        `${API_BASE}/api/anime/projects/${projectId}/characters`,
      );
      if (response.ok) {
  ...
  async function loadProjectScenes(projectId) {
    try {
      const response = await fetch(
        `${API_BASE}/api/anime/projects/${projectId}/scenes`,
      );
      if (response.ok) {
        ...
        generationHistory.value = [
          {
            ...
            duration: 3.2,
            outputPath:
              "/mnt/1TB-storage/anime-projects/unorganized/images/20251202/sample1.png",
          },
          {
            id: 2,
            ...
            duration: 2.8,
            outputPath:
              "/mnt/1TB-storage/anime-projects/unorganized/images/20251202/sample2.png",
          },
        ];
      }
    } catch (err) {
  ...
  async function loadOrganizedFiles(date = null) {
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/anime/files?date=${dateParam}`,
      );
