  ...
  <TowerCard class="p-6">
    ...
    <div v-else-if="generations && generations.length > 0" class="space-y-3">
      <div
        ...
        :key="gen.id"
        class="generation-item p-4 bg-gray-800 rounded"
      >
        <div class="flex justify-between items-start">
          ...
          <div class="text-right">
            <span
              :class="getStatusClass(gen.status)"
              class="text-sm px-2 py-1 rounded"
            >
              {{ gen.status }}
            </span>
            <div
              v-if="gen.progress !== undefined"
        ...
        <!-- Progress bar -->
        <div
          v-if="gen.status !== 'completed' && gen.status !== 'failed'"
          class="mt-3"
        >
        ...
        <div
          v-if="gen.status === 'failed' && gen.error"
    ...
    <div v-else class="text-center text-gray-400 py-8">No generations yet</div>
  </TowerCard>
</template>
...
const { data: generations, loading } = usePolling("/api/generations", {
  interval: 3000,
});
