          ...
          <TowerCard class="p-4">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-lg font-semibold">
                {{
                  selectedProject
                    ? selectedProject.name + " - Scenes"
                    : "Select a project"
                }}
              </h3>
              <TowerButton
                v-if="selectedProject && !showNewScene"
                @click="showNewScene = true"
              >
                Add Scene
              </TowerButton>
            </div>
              ...
              <div class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <TowerInput
                      v-model.number="newScene.scene_number"
                      type="number"
                    />
                  </div>
                  <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      v-model.number="newScene.duration"
                ...
                <div class="flex gap-2">
                  ...
                  <TowerButton @click="showNewScene = false"
                    >Cancel</TowerButton
                  >
                </div>
              </div>
            ...
            <div v-else-if="scenesLoading" class="text-center py-4">
              Loading scenes...
            </div>
            ...
            <div v-else-if="scenes && scenes.length > 0" class="space-y-2">
              <div
                ...
                :class="{
                  'border-2 border-blue-500': selectedScene?.id === scene.id,
                }"
                  ...
                  <div class="flex-1">
                    <div class="flex gap-2 items-center mb-1">
                      <span class="text-xs font-mono text-gray-400"
                        >Scene {{ scene.scene_number }}</span
                      >
                      <span
                        class="text-xs px-2 py-0.5 rounded"
                        :class="getStatusClass(scene.status)"
                      >
                        {{ scene.status }}
                      </span>
                      <span
                    ...
                    <p class="text-xs text-gray-400 mt-1">
                      ...
                      <span
                        v-if="scene.quality && scene.quality !== 'standard'"
                  ...
                  <div class="flex gap-2">
                    ...
                    >
                      {{
                        scene.duration <= 10
                          ? "🏠 Generate (Free)"
                          : `☁️ Generate (~$${getEstimatedCost(scene)})`
                      }}
                    </TowerButton>
                    <a
          ...
          <TowerCard class="p-4">
            ...
            <div v-if="selectedScene" class="space-y-3">
              <div>
                <label class="block text-sm mb-1">Scene #</label>
                <TowerInput
                  v-model.number="selectedScene.scene_number"
                  type="number"
                  disabled
                />
              </div>
              ...
              <TowerButton class="w-full" @click="updateScene"
                >Save</TowerButton
              >
              >
            </div>
            ...
            <div v-else-if="selectedProject" class="space-y-3">
              ...
              <TowerButton class="w-full" @click="updateProject"
                >Save Project</TowerButton
              >
            </div>
          ...
          <div class="col-span-4">
            <div class="space-y-3">
              <div>
                ...
                <TowerInput
                  v-model="soundtrackSearch"
                  placeholder="anime cyberpunk epic"
                />
              </div>
              <div>
                <label class="block text-sm mb-1">Mood</label>
                <select
                  v-model="soundtrackMood"
                  class="w-full p-2 bg-gray-800 rounded text-sm"
                >
              ...
              <TowerButton class="w-full" @click="searchSoundtracks"
                >Search</TowerButton
              >
            </div>
          ...
          <div class="col-span-4">
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto">
              ...
              >
                ...
                <p
                  v-if="track.anime_relevance_score"
          ...
          <div class="col-span-4">
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto">
              ...
              >
                ...
                <span
                  v-if="playlist.anime_relevance"
                  class="text-xs px-2 py-0.5 bg-green-900 text-green-300 rounded"
                >
                  Anime Related
                </span>
              </div>
...
const updateScene = async () => {
  try {
    const { execute } = useApi(`/api/anime/scenes/${selectedScene.value.id}`, {
      method: "PUT",
    });
...
const updateProject = async () => {
  try {
    const { execute } = useApi(
      `/api/anime/projects/${selectedProject.value.id}`,
      { method: "PUT" },
    );
  ...
  const checkStatus = async () => {
    try {
      ...
      if (status.status === "completed") {
        ...
        await createSceneCommit(
          scene,
          `Completed scene ${scene.scene_number} - ${status.message}`,
        );
      } else if (status.status === "failed") {
        scene.status = "failed";
        await updateScene();
        error(
          `Scene ${scene.scene_number} generation failed: ${status.message}`,
        );
