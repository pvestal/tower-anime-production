<template>
  <div class="echo-anime-console">
    ...
    <div v-if="currentCoordination" class="coordination-panel">
      <div class="panel-header">
        <span class="panel-title">ECHO COORDINATION ACTIVE</span>
        <span class="coordination-id">{{
          currentCoordination.orchestration_id
        }}</span>
      </div>
      <div class="coordination-content">
        <div class="coord-item">
          ...
          <span class="coord-value">{{
            currentCoordination.original_prompt
          }}</span>
        </div>
        <div class="coord-item">
          <span class="coord-label">ENHANCED:</span>
          <span class="coord-value">{{
            currentCoordination.enhanced_prompt
          }}</span>
        </div>
        <div class="coord-item">
          <span class="coord-label">CHARACTER:</span>
          <span class="coord-value">{{
            currentCoordination.character || "None"
          }}</span>
        </div>
        <div class="coord-item">
          <span class="coord-label">STATUS:</span>
          <span class="coord-value status">{{
            currentCoordination.status
          }}</span>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  ...
  if (echoConnected.value) {
    addConsoleLine(
      "success",
      "Connected to Echo Brain anime coordination system",
    );
    await loadEchoStatus();
  } else {
    addConsoleLine(
      "error",
      "Failed to connect to Echo Brain - check service status",
    );
  }
...
async function loadEchoStatus() {
  try {
    const response = await fetch("/api/anime/echo/status");
    if (response.ok) {
      const status = await response.json();
      addConsoleLine(
        "info",
        `System Status: ${status.overall_status.toUpperCase()}`,
      );
      addConsoleLine(
        "info",
        `Active Coordinators: ${status.components.echo_coordination.coordinators_loaded}`,
      );
      addConsoleLine(
        "info",
        `Characters Loaded: ${status.components.character_management.characters_loaded}`,
      );
      addConsoleLine(
        "info",
        `Learning Engine: ${status.components.style_learning.active ? "ACTIVE" : "INACTIVE"}`,
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      data.characters.forEach((char) => {
        addConsoleLine(
          "info",
          `  • ${char.name} (source: ${char.sources.join(", ")})`,
        );
      });
    } else {
...
async function handleGenerate(args) {
  if (args.length === 0) {
    ...
    addConsoleLine(
      "info",
      'Example: generate "walking in Tokyo street" "Kai Nakamura"',
    );
    return;
  }
  ...
  if (!promptMatch) {
    addConsoleLine(
      "error",
      'Prompt must be in quotes. Example: generate "walking in street"',
    );
    return;
  }
  ...
  addConsoleLine(
    "command",
    `Initiating generation with Echo Brain coordination...`,
  );
  ...
  try {
    ...
    // Use the working generation endpoint
    const response = await fetch("/api/anime/working-generate", {
      method: "POST",
    ...
    if (response.ok) {
      ...
      if (result.character_consistency?.enhanced_prompt) {
        addConsoleLine(
          "success",
          "Echo enhanced prompt with character consistency",
        );
        addConsoleLine(
          "info",
          `Enhanced: "${result.character_consistency.enhanced_prompt}"`,
        );
      }
      ...
      currentCoordination.value = {
        ...
        enhanced_prompt:
          result.character_consistency?.enhanced_prompt || prompt,
        character: character,
  ...
  const checkStatus = async () => {
    try {
      ...
      if (attempts < maxAttempts) {
        ...
        addConsoleLine(
          "warning",
          "Generation monitoring timeout - check ComfyUI manually",
        );
      }
    } catch (error) {
      addConsoleLine(
        "info",
        `Generation still processing... (${attempts}/${maxAttempts})`,
      );
      attempts++;
      if (attempts < maxAttempts) {
        setTimeout(checkStatus, 3000);
...
async function showHistory() {
  ...
  addConsoleLine(
    "info",
    "Generation history feature requires additional Echo endpoint",
  );
}
...
// Poll job status for working-generate endpoint
async function pollJobStatus(jobId) {
  ...
  const checkStatus = async () => {
    try {
      const response = await fetch(`/api/anime/job-status/${jobId}`);
      const status = await response.json();

      if (status.status === "completed") {
        addConsoleLine("success", `✅ Generation completed!`);
        addConsoleLine("info", `Output: ${status.output_path}`);
        if (currentCoordination.value) {
          currentCoordination.value.status = "completed";
        }
        return;
      } else if (status.status === "failed") {
        addConsoleLine("error", `❌ Generation failed: ${status.error}`);
        return;
      }
