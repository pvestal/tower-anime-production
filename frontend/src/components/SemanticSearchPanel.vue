<template>
  <div class="semantic-search-panel">
    ...
    <!-- Search Suggestions -->
    <div
      v-if="suggestions.length > 0 && !searching && !hasResults"
          ...
          <div class="filter-group">
            ...
            <div class="similarity-control">
              ...
              <span class="similarity-value">{{
                filters.similarityThreshold.toFixed(2)
              }}</span>
            </div>
          </div>
    ...
    <div v-if="hasResults" class="search-results">
          ...
          <Column field="thumbnail" header="" style="width: 100px">
            <template #body="slotProps">
              <div class="list-thumbnail">
                ...
                <i
                  v-else
                  :class="getContentIcon(slotProps.data.content_type)"
                ></i>
              </div>
            </template>
          </Column>
            ...
            <template #body="slotProps">
              <div class="title-cell">
                ...
                <span class="similarity"
                  >{{ Math.round(slotProps.data.similarity_score * 100) }}%
                  match</span
                >
              </div>
            </template>
      ...
      <!-- Bulk Actions -->
      <div v-if="selectedResults.length > 0" class="bulk-actions">
    ...
    <!-- Empty State -->
    <div v-if="!hasResults && !searching" class="empty-state">
      <div class="empty-content">
        <i
          class="pi pi-search"
          style="font-size: 4rem; color: #666; margin-bottom: 1rem"
        ></i>
        <h3>Semantic Search</h3>
        <p>
          Search your project assets using natural language. Echo Brain will
  ...
  try {
    const searchParams = {
      ...
      filters: {
        ...filters.value,
        project_id:
          searchScope.value === "project" ? store.selectedProject?.id : null,
      },
      top_k: 50,
    };
    ...
    if (filters.value.contentTypes.length > 0) {
      searchResults.value = searchResults.value.filter((result) =>
        filters.value.contentTypes.includes(result.content_type),
      );
    }
...
function selectResult(result) {
  ...
  if (index > -1) {
    ...
    selectedResultsObjects.value = selectedResultsObjects.value.filter(
      (r) => r.id !== result.id,
    );
  } else {
...
function exportResults() {
  ...
  const data =
    selectedResultsObjects.value.length > 0
      ? selectedResultsObjects.value
      : searchResults.value;
