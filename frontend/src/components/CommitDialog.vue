    ...
    <div class="commit-form space-y-4">
      ...
      <div class="field">
        ...
        <small class="text-gray-500"
          >Describe what changed in this scene version</small
        >
      </div>
      ...
      <div class="scene-summary p-3 bg-gray-50 rounded-lg">
        ...
        <div class="grid grid-cols-2 gap-2 text-sm">
          ...
          <div>
            <strong>Music Track:</strong> {{ sceneData.musicTrack || "None" }}
          </div>
          ...
          <div class="cost-preview p-3 border rounded-lg">
            <div class="flex justify-between items-center">
              ...
              <span class="font-mono text-lg"
                >${{ estimatedCost.toFixed(2) }}</span
              >
            </div>
            <div class="text-xs text-gray-500 mt-1">
              {{ form.renderConfig.quality }} quality •
              {{ estimatedRenderTime }}min render
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="field">
        ...
        <Chips v-model="form.tags" placeholder="Add tags..." class="w-full" />
        <small class="text-gray-500"
          >Tags help organize and find commits later</small
        >
      </div>
    </div>
...
const estimatedCost = computed(() => {
  ...
  const baseCost = (frames / 120) * 8.5; // Base cost for 120 frames
...
watch(
  ...
  (newData) => {
    if (newData && Object.keys(newData).length > 0) {
      ...
      if (newData.musicTrack)
        suggestions.push(`sync with ${newData.musicTrack}`);
