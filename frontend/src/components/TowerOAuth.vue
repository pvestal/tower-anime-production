  ...
  <div class="tower-oauth">
    ...
    <div class="auth-providers">
      ...
      >
        ...
        <span v-if="!provider.configured" class="not-configured"
          >Not configured</span
        >
      </div>
    ...
    <div v-if="appleMusicConnected" class="apple-music-status">
      ...
      <button class="refresh-button" @click="refreshAppleMusicPlaylists">
...
const loadProviders = async () => {
  try {
    ...
    oauthProviders.value = data.providers.filter(
      (p) => p.type !== "music-service",
    );
    appleMusicProvider.value = data.providers.find(
      (p) => p.name === "apple-music",
    );
  } catch (error) {
...
const checkSession = async () => {
  ...
  try {
    const response = await fetch(
      "http://localhost:8088/api/auth/session/validate",
      {
        headers: {
...
const checkAppleMusicStatus = async () => {
  ...
  try {
    const response = await fetch(
      "http://localhost:8088/api/auth/apple-music/status",
      {
        headers: {
...
const connectAppleMusic = async () => {
  ...
  try {
    ...
    const tokenResponse = await fetch(
      "http://localhost:8088/api/auth/apple-music/developer-token",
    );
    ...
    if (musicUserToken) {
      ...
      const storeResponse = await fetch(
        "http://localhost:8088/api/auth/apple-music/store-token",
        {
          method: "POST",
