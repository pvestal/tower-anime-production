  ...
  <div class="git-workflow-visualizer">
    ...
    <div class="git-header">
      ...
      <div class="git-controls">
        ...
        <button
          class="control-button secondary"
          :disabled="pushing || !hasUnpushedCommits"
          @click="performPush"
        >
    ...
    <div class="git-section">
      <div class="section-header">
        ...
        <div class="view-toggle">
          <button
            :class="{ active: timelineView === 'visual' }"
            class="toggle-button"
            @click="timelineView = 'visual'"
          >
          ...
          >
            <div class="branch-header">
              <div class="branch-info">
                ...
                <span
                  ...
                  >CURRENT</span
                >
                <span v-if="branch.ahead > 0" class="ahead-indicator"
                  >+{{ branch.ahead }}</span
                >
                <span v-if="branch.behind > 0" class="behind-indicator"
                  >-{{ branch.behind }}</span
                >
              </div>
              <div class="branch-actions">
                ...
                <button
                  v-if="branch.name !== currentBranch && !branch.isRemote"
            ...
            <div class="commit-nodes">
              <div
                ...
                :class="[
                  'commit-node',
                  { merge: commit.isMerge, unpushed: !commit.isPushed },
                ]"
        ...
        <div class="branch-list">
          ...
          >
            <div class="branch-summary">
              ...
              <div class="branch-stats">
                <span class="commit-count"
                  >{{ branch.totalCommits }} commits</span
                >
            ...
            <div class="branch-indicators">
              <span v-if="branch.ahead > 0" class="indicator ahead"
                >↑{{ branch.ahead }}</span
              >
              <span v-if="branch.behind > 0" class="indicator behind"
                >↓{{ branch.behind }}</span
              >
              <span v-if="branch.hasConflicts" class="indicator conflict"
                >!</span
              >
            </div>
          </div>
        ...
        >
          ...
          <div class="commit-details">
            ...
            <div class="commit-meta">
              ...
              <span v-if="commit.branch" class="commit-branch">{{
                commit.branch
              }}</span>
            </div>
          </div>
          ...
          <div class="commit-actions">
            <button class="action-button" @click.stop="viewCommitDiff(commit)">
              <i class="pi pi-eye"></i>
            </button>
            <button
              v-if="commit.canRevert"
            ...
            <button
              v-if="commit.branch !== currentBranch"
              class="action-button"
              @click.stop="cherryPickCommit(commit)"
            >
    ...
    <div v-if="mergeConflicts.length > 0" class="git-section">
      <div class="section-header">
        ...
        <div class="conflict-actions">
          ...
          <button class="control-button danger" @click="abortMerge">
        ...
        >
          ...
          <div class="conflict-actions">
            <button
              ...
              @click="resolveConflict(conflict, 'ours')"
            >
              Accept Ours
            </button>
            <button
            ...
            <button class="resolve-button" @click="editConflict(conflict)">
              Manual Edit
            </button>
          </div>
        </div>
    ...
    >
      <div class="branch-dialog" @click.stop>
        <div class="dialog-header">
          ...
          <button class="close-button" @click="showNewBranchDialog = false">
            <i class="pi pi-times"></i>
          </button>
        </div>
    ...
    <div
      v-if="showCommitDiff"
      class="dialog-overlay"
      @click="showCommitDiff = false"
    >
      <div class="diff-dialog" @click.stop>
        <div class="dialog-header">
          <h4>Commit {{ selectedCommit?.hash?.substring(0, 7) }}</h4>
          <button class="close-button" @click="showCommitDiff = false">
            <i class="pi pi-times"></i>
          </button>
        </div>
...
export default {
  ...
  setup() {
    ...
    const filteredCommits = computed(() => {
      ...
      if (selectedBranchForHistory.value) {
        filtered = filtered.filter(
          (commit) => commit.branch === selectedBranchForHistory.value,
        );
      }
    ...
    const cherryPickCommit = async (commit) => {
      try {
        const response = await fetch(
          `/api/git/commits/${commit.hash}/cherry-pick`,
          {
            method: "POST",
          },
        );
    ...
    const abortMerge = async () => {
      if (
        !confirm(
          "Abort the current merge? This will reset to the previous state.",
        )
      )
        return;
