    ...
    <div v-show="activeTab === 'generation'" class="tab-content generation-tab">
      <div class="generation-controls">
        <div class="control-grid">
          ...
          <div class="control-group">
            ...
            <span class="duration-display"
              >{{ generationParams.duration }}s</span
            >
          </div>
        ...
        <div class="generation-actions">
          ...
          >
            <i
              :class="isGenerating ? 'pi pi-spin pi-spinner' : 'pi pi-play'"
            ></i>
      ...
      <div v-if="isGenerating" class="generation-progress">
        <div class="progress-header">
          <span
            >Generating {{ generationParams.style.replace("_", " ") }}...</span
          >
          ...
          >
            <div class="track-header">
              ...
              <div class="track-actions">
                ...
                <button class="download-btn" @click="downloadTrack(track)">
    ...
    <div v-show="activeTab === 'analysis'" class="tab-content analysis-tab">
      <div class="analysis-controls">
        ...
        <div class="analysis-options">
          <label>
            <input v-model="analysisOptions.detectKey" type="checkbox" />
            Detect Musical Key
          </label>
          <label>
            <input v-model="analysisOptions.detectBeats" type="checkbox" />
            Extract Beat Markers
          </label>
          <label>
            <input v-model="analysisOptions.energyAnalysis" type="checkbox" />
        ...
        >
          <i
            :class="isAnalyzing ? 'pi pi-spin pi-spinner' : 'pi pi-chart-bar'"
          ></i>
      ...
      <div v-if="analysisResults" class="analysis-results">
        ...
        <div class="results-grid">
          ...
          <div v-if="analysisResults.key" class="result-card">
            <div class="result-label">Musical Key</div>
            <div class="result-value">{{ analysisResults.key }}</div>
          </div>

          <div v-if="analysisResults.energy" class="result-card">
            <div class="result-label">Energy Level</div>
            <div class="result-value">
              {{ analysisResults.energy.toFixed(0) }}%
            </div>
            <div class="energy-bar">
              <div
                class="energy-fill"
                :style="{ width: analysisResults.energy + '%' }"
              ></div>
            </div>
          </div>

          <div v-if="analysisResults.tempo_class" class="result-card">
        ...
        <div v-if="analysisResults.beats" class="beats-visualization">
          ...
          <div class="beats-timeline">
            <div
              ...
              :style="{
                left: (beat.time / analysisResults.duration) * 100 + '%',
              }"
            ></div>
          </div>
        </div>
      ...
      <div class="apple-music-controls">
        <div class="search-section">
          <div class="search-input-group">
            ...
            <button class="search-btn" @click="searchAppleMusic">
          ...
          <div class="results-list">
            ...
            >
              ...
              <div class="track-meta">
                ...
                <div v-if="track.relevance_score" class="track-relevance">
                  {{ track.relevance_score }}% match
                </div>
              </div>
              <div class="track-actions">
                <button class="preview-btn" @click="previewAppleTrack(track)">
                  <i class="pi pi-play"></i>
                </button>
                <button class="analyze-btn" @click="analyzeAppleTrack(track)">
    ...
    <div v-show="activeTab === 'sync'" class="tab-content sync-tab">
      <div class="sync-controls">
        <div class="sync-setup">
          ...
          <button
            :disabled="
              !syncParams.videoFile || !syncParams.audioFile || isSyncing
            "
            class="sync-btn primary"
            @click="syncVideoAudio"
          >
            <i
              :class="isSyncing ? 'pi pi-spin pi-spinner' : 'pi pi-refresh'"
            ></i>
        ...
        <div v-if="syncResults" class="sync-results">
          ...
          <div class="result-actions">
            <button class="preview-btn" @click="previewSyncedVideo">
              <i class="pi pi-play"></i> Preview Result
            </button>
            <button class="jellyfin-btn" @click="addToJellyfin">
...
const analyzeAppleTrack = async (track) => {
  try {
    const response = await fetch(
      `http://127.0.0.1:8315/api/apple-music/track/${track.id}/analysis`,
    );
    if (response.ok) {
...
const addToJellyfin = async () => {
  ...
  try {
    const response = await fetch(
      "http://127.0.0.1:8308/api/jellyfin/add-track",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
onMounted(async () => {
  ...
  availableVideos.value = [
    {
      name: "Anime Scene 1",
      path: "/mnt/1TB-storage/ComfyUI/output/anime_30sec_final_00006.mp4",
    },
    {
      name: "Echo Video",
      path: "/mnt/1TB-storage/ComfyUI/output/echo_video_877c3aba_00001.mp4",
    },
  ];
  ...
  availableAudio.value = [
    {
      name: "Epic Track 1",
      path: "/opt/tower-music-production/generated/epic_track_001.wav",
    },
    {
      name: "Cyberpunk Beat",
      path: "/opt/tower-music-production/generated/cyberpunk_beat.wav",
    },
  ];
});
</script>
