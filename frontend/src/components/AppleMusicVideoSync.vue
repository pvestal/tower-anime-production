  ...
  <div class="apple-music-video-sync">
    ...
    <div v-else class="generation-step">
      ...
      <div class="selected-playlist">
        ...
        <button class="change-btn" @click="selectedPlaylist = null">
          Change Playlist
        </button>
      </div>
...
const connectAppleMusic = async () => {
  ...
  try {
    ...
    const tokenResponse = await fetch("/api/auth/apple-music/developer-token");
    ...
    if (musicUserToken.value) {
      ...
      const storeResponse = await fetch("/api/auth/apple-music/authorize", {
        method: "POST",
...
const loadPlaylists = async () => {
  ...
  try {
    const response = await fetch("/api/apple-music/playlists", {
      headers: {
...
const selectPlaylist = async (playlist) => {
  ...
  try {
    const response = await fetch(
      `/api/apple-music/playlist/${playlist.id}/tracks`,
      {
        headers: {
          "Music-User-Token": musicUserToken.value,
        },
...
const generateVideo = async (track) => {
  ...
  try {
    ...
    const response = await fetch("/api/anime/generate-with-music", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        track: {
...
onMounted(async () => {
  try {
    const response = await fetch("/api/auth/apple-music/status");
    const data = await response.json();
