  ...
  <div class="echo-assistant-panel">
    ...
    <div class="assistant-header">
      ...
      <div class="assistant-controls">
        <div
          class="echo-status"
          :class="{ connected: echoConnected, thinking: echoThinking }"
        >
        ...
        >
              ...
              <div class="impact-metrics">
                <div v-if="suggestion.impact.quality" class="impact-item">
                  ...
                  <span
                    :class="[
                      'impact-value',
                      getImpactClass(suggestion.impact.quality),
                    ]"
                  >
                ...
                <div v-if="suggestion.impact.performance" class="impact-item">
                  ...
                  <span
                    :class="[
                      'impact-value',
                      getImpactClass(suggestion.impact.performance),
                    ]"
                  >
                ...
                <div v-if="suggestion.impact.time" class="impact-item">
                  ...
                  <span
                    :class="[
                      'impact-value',
                      getImpactClass(suggestion.impact.time),
                    ]"
                  >
          ...
          <div class="suggestion-actions">
            <button
              ...
              :disabled="
                suggestion.applied || applyingIds.includes(suggestion.id)
              "
              @click="applySuggestion(suggestion)"
            >
              <i
                :class="
                  applyingIds.includes(suggestion.id)
            ...
            <button
              class="suggestion-button explain"
              @click="explainSuggestion(suggestion)"
            >
    ...
    <div class="assistant-section">
      <div class="section-header">
        ...
        <button class="control-button secondary" @click="clearChat">
      ...
      <div class="chat-container">
        <div ref="chatMessages" class="chat-messages">
          ...
          >
            <div class="message-avatar">
              <i
                :class="message.role === 'user' ? 'pi pi-user' : 'pi pi-robot'"
              ></i>
            </div>
        ...
        <div class="chat-input">
          ...
          >
            <i
              :class="echoThinking ? 'pi pi-spin pi-spinner' : 'pi pi-send'"
            ></i>
          </button>
        </div>
      </div>
    ...
    >
      <div class="settings-dialog" @click.stop>
        <div class="dialog-header">
          ...
          <button class="close-button" @click="showSettingsDialog = false">
            <i class="pi pi-times"></i>
          </button>
        </div>
        ...
        <div class="dialog-content">
          <div class="setting-group">
            ...
            <select
              v-model="settings.suggestionFrequency"
          ...
          <div class="setting-group">
            ...
            <input
              v-model="settings.autoApplyLowRisk"
              type="checkbox"
          ...
          <div class="setting-group">
            ...
            <input
              v-model="settings.showReasoning"
              type="checkbox"
              class="setting-checkbox"
            />
          </div>
        ...
        <div class="dialog-actions">
          ...
          <button class="dialog-button primary" @click="saveSettings">
            Save Settings
          </button>
        </div>
      </div>
  ...
  setup() {
    ...
    const filteredSuggestions = computed(() => {
      ...
      if (selectedPriority.value) {
        filtered = filtered.filter(
          (s) => s.priority === selectedPriority.value,
        );
      }
    ...
    const refreshSuggestions = async () => {
      loading.value = true;
      try {
        await Promise.all([checkEchoConnection(), loadSuggestions()]);
      } finally {
        loading.value = false;
      }
    };
    ...
    const optimizeParameters = async () => {
      try {
        ...
        if (response.ok) {
          ...
          addChatMessage(
            "echo",
            `I've analyzed your current parameters and suggest the following optimizations:\n\n${result.summary}`,
          );
        ...
        addChatMessage(
          "echo",
          "Sorry, I encountered an error while optimizing parameters.",
        );
      } finally {
    ...
    const analyzeProject = async () => {
      try {
        ...
        addChatMessage(
          "echo",
          "Sorry, I encountered an error during project analysis.",
        );
      } finally {
        echoThinking.value = false;
      }
    ...
    const generateIdeas = async () => {
      try {
        ...
        addChatMessage(
          "echo",
          "Sorry, I encountered an error while generating ideas.",
        );
      } finally {
        echoThinking.value = false;
      }
    ...
    const checkConsistency = async () => {
      try {
        ...
        addChatMessage(
          "echo",
          "Sorry, I encountered an error during consistency check.",
        );
      } finally {
        echoThinking.value = false;
      }
    ...
    const applySuggestion = async (suggestion) => {
      ...
      try {
        const response = await fetch(
          `/api/echo/suggestions/${suggestion.id}/apply`,
          {
            method: "POST",
          },
        );
        ...
        if (response.ok) {
          ...
          addChatMessage(
            "echo",
            `Applied suggestion: "${suggestion.title}". Your settings have been updated.`,
          );
        } else {
          addChatMessage(
            "echo",
            `Failed to apply suggestion: "${suggestion.title}". Please try again.`,
          );
        }
      } catch (error) {
        addChatMessage("echo", `Error applying suggestion: ${error.message}`);
      } finally {
        applyingIds.value = applyingIds.value.filter(
          (id) => id !== suggestion.id,
        );
      }
    };
    ...
    const explainSuggestion = async (suggestion) => {
      try {
        echoThinking.value = true;
        const response = await fetch(
          `/api/echo/suggestions/${suggestion.id}/explain`,
          {
            method: "POST",
          },
        );
        ...
        if (response.ok) {
          ...
          addChatMessage(
            "echo",
            `**Explanation for "${suggestion.title}":**\n\n${result.explanation}`,
          );
        }
      } catch (error) {
        addChatMessage(
          "echo",
          "Sorry, I couldn't provide an explanation for that suggestion.",
        );
      } finally {
        echoThinking.value = false;
      }
    ...
    onMounted(() => {
      ...
      addChatMessage(
        "echo",
        "Hello! I'm Echo, your AI assistant for anime production. I can help optimize your parameters, suggest improvements, and answer questions about your workflow. How can I help you today?",
      );
    });
