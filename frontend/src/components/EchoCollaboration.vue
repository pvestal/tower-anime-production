<template>
  <div class="echo-collaboration">
    ...
    <div class="echo-status-bar">
      <div
        class="echo-indicator"
        :class="{ connected: echoConnected, active: echoActive }"
      >
        <i class="pi pi-circle-fill"></i>
        <span
          >Echo Brain {{ echoConnected ? "Connected" : "Disconnected" }}</span
        >
      </div>
      <div class="echo-mode">
        <Tag
          :value="echoMode"
          :severity="echoMode === 'Production Director' ? 'success' : 'info'"
        />
      </div>
    </div>

    <!-- Real-time Echo Feedback -->
    <Card v-if="currentEchoFeedback" class="echo-feedback-card">
      <template #title>
        <i class="pi pi-brain"></i> Echo Brain Analysis
      </template>
      <template #content>
        <div class="echo-message">{{ currentEchoFeedback.message }}</div>
        <div v-if="currentEchoFeedback.suggestions" class="echo-suggestions">
          <h4>Suggestions:</h4>
          <ul>
            <li
              v-for="suggestion in currentEchoFeedback.suggestions"
              :key="suggestion"
            >
              {{ suggestion }}
            </li>
    ...
    <Card v-if="showCoordinator" class="echo-coordinator">
      <template #title>
      ...
      <template #content>
        <div class="coordinator-section">
          ...
          <div v-if="characterAnalysis" class="character-analysis">
            <p>
              <strong>Character:</strong> {{ characterAnalysis.character_name }}
            </p>
            <p>
              <strong>Consistency Score:</strong>
              {{ characterAnalysis.consistency_score }}/1.0
            </p>
            <div class="visual-traits">
              <Chip
                v-for="trait in characterAnalysis.visual_traits"
                :key="trait"
                :label="trait"
              />
            </div>
        ...
        <div class="coordinator-section">
          ...
          <div v-if="stylePreferences" class="style-preferences">
            <div
              v-for="(value, key) in stylePreferences"
              :key="key"
        ...
        <div class="coordinator-section">
          ...
          <div v-if="currentGeneration" class="generation-status">
            ...
            <div v-if="currentGeneration.echo_insights" class="echo-insights">
              <strong>Echo Insights:</strong>
              {{ currentGeneration.echo_insights }}
            </div>
          </div>
        </div>
      </template>
    ...
    <!-- Echo Chat Interface -->
    <Card v-if="showChat" class="echo-chat">
      <template #title>
        ...
        <div ref="chatMessages" class="chat-messages">
          ...
          >
            <div class="message-header">
              <strong>{{
                message.sender === "user" ? "You" : "Echo Brain"
              }}</strong>
...
async function connectToEcho() {
  try {
    // Check Echo health
    const healthResponse = await fetch("/api/echo/health", {
      method: "GET",
...
function initializeWebSocket() {
  ...
  try {
    // Build domain-agnostic WebSocket URL
    const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
    const host = window.location.host;
    echoSocket.value = new WebSocket(
      `${protocol}//${host}/api/echo/studio/collaborate`,
    );
...
async function loadEchoState() {
  try {
    ...
    if (props.selectedScene?.characters) {
      const charResponse = await fetch(`/api/echo/anime/characters`, {
        method: "GET",
      });
      if (charResponse.ok) {
    ...
    const prefResponse = await fetch(`/api/echo/anime/preferences/summary`, {
      method: "GET",
    });
    if (prefResponse.ok) {
...
defineExpose({
  requestEchoFeedback: async (prompt, character) => {
    ...
    try {
      const response = await fetch("/api/echo/anime/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  startEchoGeneration: async (generationRequest) => {
    ...
    try {
      const response = await fetch("/api/echo/anime/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
