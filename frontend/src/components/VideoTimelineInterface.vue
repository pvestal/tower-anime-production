  ...
  <div class="video-timeline-interface cyberpunk-theme">
    ...
    <div class="timeline-header">
      ...
      <div class="timeline-tools">
        <button
          class="cp-button"
          :class="{ 'neon-cyan': musicSyncEnabled }"
          @click="toggleMusicSync"
        >
          <i class="pi pi-volume-up"></i> Music Sync
        </button>
        <button
          class="cp-button"
          :class="{ 'neon-cyan': beatMarkersEnabled }"
          @click="toggleBeatMarkers"
        >
    ...
    <div ref="timelineContainer" class="timeline-container">
        ...
        <div class="track-header">
          ...
          <div class="track-controls">
            <button class="track-button" @click="toggleVideoTrack">
              <i
                :class="videoTrackEnabled ? 'pi pi-eye' : 'pi pi-eye-slash'"
              ></i>
            </button>
        ...
        <div class="track-content">
          ...
          >
            ...
            <div
              ...
              @mousedown="startResize(clip, 'left', $event)"
            ></div>
            <div
              class="clip-resize-handle right"
              @mousedown="startResize(clip, 'right', $event)"
            ></div>
          </div>
        </div>
      </div>
      ...
      <div class="timeline-track audio-track">
        <div class="track-header">
          ...
          <div class="track-controls">
            <button class="track-button" @click="toggleAudioTrack">
              <i
                :class="
                  audioTrackEnabled ? 'pi pi-volume-up' : 'pi pi-volume-off'
                "
              ></i>
            </button>
        ...
        <div class="track-content">
          <!-- Waveform Display -->
          <div v-if="audioWaveform" class="waveform-container">
            <canvas
              ref="waveformCanvas"
          ...
          >
            ...
            <div class="clip-info">
              <div class="clip-name">{{ clip.name }}</div>
              <div v-if="clip.bpm" class="clip-bpm">{{ clip.bpm }} BPM</div>
            </div>
          </div>
        ...
        <div class="track-content">
          ...
          >
            <div class="marker-avatar">
              <img
                :src="marker.character.avatarUrl"
                :alt="marker.character.name"
              />
            </div>
    ...
    <div v-if="musicSyncEnabled" class="music-sync-panel cp-fade-in">
      <div class="sync-header">
        ...
        <div class="sync-header-controls">
          ...
          <button
            class="cp-button"
            :class="{ 'neon-cyan': advancedModeEnabled }"
      ...
      <div class="bpm-visualizer-container">
        <EnhancedBPMVisualizer
          :audio-file="currentAudioFile"
          :video-duration="totalDuration"
          :auto-analyze="true"
          :show-manual-controls="advancedModeEnabled"
          @bpm-detected="onBPMDetected"
          @beat-marker="onBeatMarker"
          @analysis-complete="onAnalysisComplete"
        />
      </div>

      <!-- Video-Audio Sync Controls -->
      <div v-if="advancedModeEnabled" class="video-sync-controls">
        <div class="sync-section">
            ...
            >
              <div class="event-info">
                <span class="event-type"
                  >{{ getEventIcon(event.type) }} {{ event.name }}</span
                >
              ...
              <div class="event-controls">
                <button class="preview-btn" @click="previewEvent(event)">
          ...
          <div class="add-event-controls">
            ...
            <button class="add-event-btn" @click="addSyncEvent">
      ...
      <div v-if="!advancedModeEnabled" class="basic-sync-controls">
        ...
        <div class="sync-options">
          <label>
            <input v-model="snapToBeats" type="checkbox" />
            Snap clips to beats
          </label>
          <label>
            <input v-model="highlightDownbeats" type="checkbox" />
            Highlight downbeats
          </label>
          <label>
            <input v-model="autoSyncEffects" type="checkbox" />
    ...
    <!-- Properties Panel -->
    <div v-if="selectedClip || selectedMarker" class="properties-panel">
      ...
      <div v-if="selectedClip" class="clip-properties">
        ...
        <div v-if="selectedClip.type === 'audio'" class="property-group">
          <label>Volume</label>
          <input
        ...
        <div v-if="selectedClip.bpm" class="property-group">
          <label>BPM</label>
          <input
            v-model="selectedClip.bpm"
            type="number"
            class="cp-input"
            readonly
          />
...
function fastForward() {
  if (snapToBeats.value && beatMarkers.value.length > 0) {
    const nextBeat = beatMarkers.value.find(
      (beat) => beat.time > currentTime.value,
    );
    if (nextBeat) {
...
const syncVideoToMusic = async () => {
  ...
  try {
    ...
    const response = await fetch("http://127.0.0.1:8308/api/sync/video-audio", {
      ...
      body: JSON.stringify({
        video_path:
          videoClips.value[0]?.path ||
          "/mnt/1TB-storage/ComfyUI/output/anime_30sec_final_00006.mp4",
