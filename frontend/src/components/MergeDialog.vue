    ...
    <div class="merge-form space-y-4">
      ...
      <div class="field">
        ...
        <small class="text-gray-500"
          >Choose the branch you want to merge into {{ currentBranch }}</small
        >
      </div>
      ...
      <div v-if="selectedSourceBranch && branchComparison" class="comparison">
              ...
              <div class="stats space-y-2">
                <div class="flex justify-between">
                  ...
                  <span class="font-mono">{{
                    branchComparison.source.commits
                  }}</span>
                </div>
                <div class="flex justify-between">
                  <span>Scenes:</span>
                  <span class="font-mono">{{
                    branchComparison.source.scenes
                  }}</span>
                </div>
                <div class="flex justify-between">
                  <span>Last Modified:</span>
                  <span class="text-sm">{{
                    formatDate(branchComparison.source.lastModified)
                  }}</span>
                </div>
              </div>
            </template>
            ...
            <template #content>
              <div class="stats space-y-2">
                <div class="flex justify-between">
                  <span>Commits:</span>
                  <span class="font-mono">{{
                    branchComparison.target.commits
                  }}</span>
                </div>
                <div class="flex justify-between">
                  <span>Scenes:</span>
                  <span class="font-mono">{{
                    branchComparison.target.scenes
                  }}</span>
                </div>
                <div class="flex justify-between">
                  <span>Last Modified:</span>
                  <span class="text-sm">{{
                    formatDate(branchComparison.target.lastModified)
                  }}</span>
                </div>
              </div>
            </template>
        ...
        <div class="changes-summary p-3 border rounded-lg">
          ...
          <div class="changes space-y-1">
            <div
              v-for="change in branchComparison.changes"
              :key="change.id"
              class="change-item flex items-center gap-2 text-sm"
            >
              <i
                :class="[
        ...
        <!-- Potential Conflicts -->
        <div
          v-if="branchComparison.conflicts.length > 0"
          class="conflicts p-3 bg-yellow-50 border border-yellow-200 rounded-lg"
        >
          ...
          <div class="conflicts-list space-y-1">
            <div
              v-for="conflict in branchComparison.conflicts"
              :key="conflict.id"
              class="conflict-item text-sm text-yellow-700"
            >
          ...
          <div class="text-sm text-yellow-600 mt-2">
            These conflicts will need to be resolved manually if they occur
            during merge.
          </div>
        </div>
      </div>
      ...
      <div class="options space-y-2">
        <div class="flex items-center gap-2">
          <Checkbox id="squash" v-model="mergeOptions.squash" />
          <label for="squash" class="text-sm"
            >Squash commits into single commit</label
          >
        </div>

        <div class="flex items-center gap-2">
          <Checkbox id="delete-branch" v-model="mergeOptions.deleteBranch" />
          <label for="delete-branch" class="text-sm"
            >Delete source branch after merge</label
          >
        </div>

        <div class="flex items-center gap-2">
          <Checkbox id="create-backup" v-model="mergeOptions.createBackup" />
          <label for="create-backup" class="text-sm"
            >Create backup before merge</label
          >
        </div>
      </div>
      ...
      <div class="field">
        ...
        />
        <small class="text-gray-500"
          >Optional custom message for the merge commit</small
        >
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-between items-center">
        <div class="text-sm text-gray-500">
          ...
          {{
            branchComparison?.conflicts.length > 0
...
const mergeStrategies = [
  {
    ...
    label: "Auto Merge",
    description:
      "Automatically resolve simple conflicts, prompt for complex ones",
    icon: "pi pi-bolt",
  },
  {
    value: "manual",
      ...
      changes: [
        {
          id: 1,
          type: "added",
          description: "Added new character introduction scene",
        },
        {
          id: 2,
          type: "modified",
          description: "Enhanced battle sequence choreography",
        },
        {
          id: 3,
          type: "added",
          description: "New background music for emotional scenes",
        },
        {
          id: 4,
          type: "modified",
          description: "Updated character dialogue in finale",
        },
      ],
      conflicts: [
        {
...
const handleMerge = () => {
  if (canMerge.value) {
    const mergeData = {
      ...
      message:
        mergeMessage.value ||
        `Merge ${selectedSourceBranch.value} into ${props.currentBranch}`,
