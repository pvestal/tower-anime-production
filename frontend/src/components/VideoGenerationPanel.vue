<template>
  <div class="video-generation-panel">
    ...
    <!-- Main Form -->
    <form class="generation-form" @submit.prevent="startVideoGeneration">
      <!-- Prompt Section -->
        ...
        <div class="form-group">
          ...
          <div class="duration-controls">
            ...
            <div class="duration-display">
              {{ (form.frames / form.fps).toFixed(1) }}s ({{ form.frames }}
              frames)
            </div>
          </div>
        </div>
      ...
      <div v-if="completedVideo" class="result-section">
        <div class="result-header">
          ...
          <div class="generation-stats">
            Duration: {{ completedVideo.duration }}s | Size:
            {{ completedVideo.frames }} frames | Quality:
            {{ completedVideo.quality }}
          </div>
        </div>
        ...
        <div class="result-actions">
          <button class="action-button download" @click="downloadVideo">
            💾 Download
          </button>
          <button class="action-button share" @click="shareVideo">
            🔗 Share
          </button>
          <button class="action-button secondary" @click="generateAnother">
            🔄 Generate Another
          </button>
        </div>
      </div>
    ...
    <transition name="slide-down">
      <div v-if="error" class="error-section">
        ...
        <button class="error-close" @click="clearError">✕ Dismiss</button>
      </div>
    </transition>
  </div>
</template>
    ...
    const startVideoGeneration = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.detail || "Failed to start video generation",
          );
        }
      ...
      if (navigator.share && completedVideo.value) {
        try {
          ...
          navigator.clipboard.writeText(
            window.location.origin + completedVideo.value.url,
          );
