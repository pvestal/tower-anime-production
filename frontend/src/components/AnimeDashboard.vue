            ...
            >
              ...
              <div class="project-info">
                ...
                <div class="project-stats">
                  <span class="stat"
                    >{{ project.characterCount || 0 }} chars</span
                  >
                  <span class="stat"
                    >{{ project.generationCount || 0 }} images</span
                  >
                </div>
              </div>
            </div>
          ...
          <div class="characters-grid">
            ...
            >
              ...
              <div class="character-info">
                ...
                <div
                  v-if="character.consistencyScore"
                  class="consistency-score"
                >
                  <i
                    class="pi pi-check-circle"
                    :class="getConsistencyClass(character.consistencyScore)"
                  ></i>
                  <span
                    >{{ (character.consistencyScore * 100).toFixed(0) }}%</span
                  >
                </div>
              </div>
            </div>
...
const recentGenerations = computed(() =>
  animeStore.recentGenerations.slice(0, 8),
);
const wsConnected = computed(() => animeStore.wsConnected);

// Performance metrics
const avgGenerationTime = computed(() => {
  const completed = animeStore.generationHistory.filter(
    (g) => g.status === "completed" && g.duration,
  );
  if (completed.length === 0) return "3.0";
  const avg =
    completed.reduce((sum, g) => sum + g.duration, 0) / completed.length;
...
const successRate = computed(() => {
  ...
  const successful = animeStore.generationHistory.filter(
...
const getImageUrl = (path) => {
  // Convert file path to accessible URL via anime API static mounts
  if (path && path.includes("/mnt/1TB-storage/ComfyUI/output")) {
    return path.replace("/mnt/1TB-storage/ComfyUI/output", "/anime-output");
  }
  if (path && path.includes("/opt/tower-anime-production/references")) {
    return path.replace(
      "/opt/tower-anime-production/references",
      "/anime-references",
    );
  }
  return path;
