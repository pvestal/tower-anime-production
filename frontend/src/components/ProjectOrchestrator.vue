<template>
  <div class="project-orchestrator">
    ...
    <div v-if="selectedProject" class="context-display">
        ...
        <template #header>
          <div class="context-header">
            ...
            <Tag
              ...
              :severity="getStatusSeverity(selectedProject.status)"
            />
          </div>
        </template>
        <template #content>
          <div class="context-grid">
            ...
            <div class="context-item">
              ...
              <span class="context-value"
                >{{ projectCharacters.length }} defined</span
              >
            </div>
            <div class="context-item">
              <span class="context-label">Scenes:</span>
              <span class="context-value"
                >{{ projectScenes.length }} total</span
              >
            </div>
          </div>
        </template>
    ...
    <!-- Generation Queue -->
    <div v-if="generationQueue.length > 0" class="generation-queue">
      <h3>
        ...
        <Card v-for="item in generationQueue" :key="item.id" class="queue-item">
          <template #content>
            <div class="queue-item-header">
              ...
              <Tag
                :value="item.status"
                :severity="getGenerationSeverity(item.status)"
              />
            </div>
    ...
    <div v-if="lastOrchestration" class="orchestration-results">
      <h3>
      ...
      <Card class="result-card">
        <template #content>
          ...
          <div class="result-details">
            ...
            <div v-if="lastOrchestration.parameters" class="result-parameters">
              <strong>Generated Parameters:</strong>
              <pre class="parameters-code">{{
                JSON.stringify(lastOrchestration.parameters, null, 2)
              }}</pre>
            </div>
    ...
    >
      <div class="settings-content">
        <div class="setting-group">
          ...
          />
          <small
            >Higher levels use more powerful models for complex tasks</small
          >
        </div>
...
const currentTimelineBranch = computed(
  () => store.currentTimelineBranch || "main",
);
const projectCharacters = computed(
  () =>
    store.characters.filter(
      (c) => c.project_id === selectedProject.value?.id,
    ) || [],
);
const projectScenes = computed(
  () =>
    store.scenes.filter((s) => s.project_id === selectedProject.value?.id) ||
    [],
);
...
async function refreshEchoStatus() {
  ...
  try {
    ...
    toast.add({
      ...
      summary: "Echo Status",
      detail: echoConnected.value
        ? "Echo Brain is online"
        : "Echo Brain connection issues",
      life: 3000,
    });
  } catch (error) {
