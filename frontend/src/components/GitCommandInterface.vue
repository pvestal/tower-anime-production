<template>
  <div class="git-interface">
    ...
    <MergeDialog
      ...
      :available-branches="
        availableBranches.filter((b) => b.name !== currentBranch)
      "
      :current-branch="currentBranch"
      @merge="handleMerge"
    />
...
const lastCommit = computed(
  () => commitHistory.value[0] || { hash: "initial" },
);
...
const loadCurrentSceneData = async () => {
  try {
    const response = await fetch(
      `/api/anime/projects/${currentProject.value.id}/current-scene`,
    );
    if (response.ok) {
...
const deleteBranch = async (branchName) => {
  if (
    confirm(
      `Are you sure you want to delete branch "${branchName}"? This cannot be undone.`,
    )
  ) {
    try {
      ...
      if (response.ok) {
        availableBranches.value = availableBranches.value.filter(
          (b) => b.name !== branchName,
        );
        toast.add({
          severity: "success",
...
const revertToCommit = async (commitHash) => {
  if (
    confirm(
      `Revert to commit ${commitHash.substring(0, 8)}? This will undo all changes after this commit.`,
    )
  ) {
    try {
...
const loadProjectData = async () => {
  ...
  try {
    const response = await fetch(
      `/api/anime/projects/${currentProject.value.id}/git-status`,
    );
    if (response.ok) {
