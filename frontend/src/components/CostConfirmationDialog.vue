  ...
  >
    <div class="cost-breakdown">
      ...
      <div class="budget-status mb-4">
        <div class="flex justify-between items-center mb-2">
          ...
          <span class="text-sm text-gray-600"
            >${{ budgetUsed.toFixed(2) }} used</span
          >
        </div>
        <ProgressBar
      ...
      <div class="cost-comparison grid grid-cols-2 gap-4 mb-4">
        ...
        <Card
          class="firebase-option"
          :class="{ selected: selectedOption === 'firebase' }"
        >
          <template #header>
            ...
            <div class="cost-details">
              <div class="cost-item">
                ...
                <span class="font-mono"
                  >${{ costs.firebase.gpu.toFixed(2) }}</span
                >
              </div>
              <div class="cost-item">
                <span>Storage ({{ estimatedFileSize }}MB):</span>
                <span class="font-mono"
                  >${{ costs.firebase.storage.toFixed(2) }}</span
                >
              </div>
              <div class="cost-item">
                <span>Network Transfer:</span>
                <span class="font-mono"
                  >${{ costs.firebase.network.toFixed(2) }}</span
                >
              </div>
              <div class="cost-item">
                <span>Function Overhead:</span>
                <span class="font-mono"
                  >${{ costs.firebase.functions.toFixed(2) }}</span
                >
              </div>
              <hr class="my-2" />
          ...
          <template #content>
            <div class="cost-details">
              ...
              <div class="cost-item">
                ...
                <span class="font-mono"
                  >${{ costs.local.electricity.toFixed(2) }}</span
                >
              </div>
              <hr class="my-2" />
              <div class="cost-total">
                <span class="font-bold">Total:</span>
                <span class="font-bold text-lg"
                  >${{ costs.local.total.toFixed(2) }}</span
                >
              </div>
              <div class="status mt-3">
                <div
                  class="text-sm"
                  :class="
                    localGpuStatus.busy ? 'text-red-600' : 'text-green-600'
                  "
                >
                  {{ localGpuStatus.busy ? "⚠️ GPU Busy" : "✓ GPU Available" }}
                </div>
                <div class="text-sm">
                  VRAM: {{ localGpuStatus.vramUsed }}GB /
                  {{ localGpuStatus.vramTotal }}GB
                </div>
                <div class="text-sm">
                  Queue: {{ localGpuStatus.queueLength }} jobs
                </div>
                <div class="text-sm">
                  ETA: ~{{ localGpuStatus.estimatedWait }}min
                </div>
              </div>
            </div>
          </template>
      ...
      <div
        v-if="budgetExceeded"
        class="budget-warning mb-4 p-3 bg-red-100 border border-red-300 rounded"
      >
        ...
        <p class="text-sm text-red-600 mt-1">
          This render would exceed your daily $150 budget. Consider using local
          GPU or reducing quality.
        </p>
      </div>

      <div
        v-if="recommendLocal"
        class="recommendation mb-4 p-3 bg-yellow-100 border border-yellow-300 rounded"
      >
...
// Computed for v-model visible to avoid prop mutation
...
// Budget tracking
const dailyBudgetLimit = 150.0;
const budgetUsed = ref(0);
const autoApprovalThreshold = ref(5.0);
...
const budgetUsedPercent = computed(
  () => (budgetUsed.value / dailyBudgetLimit) * 100,
);
const budgetExceeded = computed(
  () => budgetUsed.value + costs.value.firebase.total > dailyBudgetLimit,
);
const recommendLocal = computed(() => {
  return (
    !localGpuStatus.value.busy &&
    costs.value.firebase.total > 10.0 &&
    budgetUsedPercent.value > 50
  );
});
const localRecommendationReason = computed(() => {
  if (budgetUsedPercent.value > 80)
    return "Budget usage is high - save costs with local rendering";
  if (costs.value.firebase.total > 20)
    return "High render cost - local GPU would be free";
...
const calculateCosts = () => {
  ...
  const resolutionMultiplier = resolution.includes("1024")
    ? 1
  ...
  costs.value.firebase.storage =
    (estimatedFileSize.value / 1024) * 0.026 * (1 / 30); // Per day storage
  costs.value.firebase.network =
    estimatedFileSize.value > 1000
      ? (estimatedFileSize.value / 1024) * 0.12
      : 0;
  costs.value.firebase.functions =
    0.0000025 * estimatedRenderTime.value * 60 * 8; // 8GB memory
  costs.value.firebase.total = Object.values(costs.value.firebase).reduce(
    (sum, cost) => sum + cost,
    0,
  );
  ...
  costs.value.local.electricity =
    (gpuWatts / 1000) * renderTimeHours * electricityRate;
  costs.value.local.total = costs.value.local.electricity;

  // Auto-select cheaper option
  selectedOption.value =
    costs.value.local.total < costs.value.firebase.total ? "local" : "firebase";
};
...
const loadBudgetStatus = async () => {
  try {
    ...
    if (response.ok) {
      ...
      autoApprovalThreshold.value = data.autoApprovalThreshold || 5.0;
    }
  } catch (error) {
...
const loadGpuStatus = async () => {
  try {
    const response = await fetch("/comfyui/system_stats");
    if (response.ok) {
      const data = await response.json();
