  ...
  <div class="project-bible-interface">
    ...
    <div class="bible-header">
      ...
      <div class="bible-controls">
        ...
        <button class="control-button secondary" @click="exportBible">
          <i class="pi pi-download"></i>
          Export
        </button>
        <button class="control-button secondary" @click="importBible">
    ...
    <div v-if="activeTab === 'characters'" class="tab-content">
      <div class="characters-section">
        ...
        <div class="search-controls">
          ...
          <button class="refresh-button" @click="refreshCharacters">
            ...
            <div class="table-cell">
              <div class="consistency-meter">
                ...
                <span class="consistency-text"
                  >{{ character.consistency_score || 0 }}%</span
                >
              </div>
            </div>
            <div class="table-cell">
              ...
              <button
                class="action-button"
                @click.stop="generateCharacterVariant(character)"
              >
        ...
        <div v-if="selectedCharacter" class="character-details">
          ...
          <button class="save-button" @click="saveCharacterDetails">
      ...
      <div class="world-section">
        <div class="world-categories">
          <button
            ...
            :class="[
              'category-button',
              { active: activeWorldCategory === category.id },
            ]"
        ...
        <div class="world-content">
          <div class="world-editor">
            <div class="editor-toolbar">
              <button class="format-button" @click="formatText('bold')">
                <i class="pi pi-bold"></i>
              </button>
              <button class="format-button" @click="formatText('italic')">
                <i class="pi pi-italic"></i>
              </button>
              <button class="format-button" @click="insertTemplate">
        ...
        <div class="consistency-results">
          ...
          >
            ...
            <div class="issue-actions">
              <button class="fix-button" @click="applyFix(issue)">
                Apply Fix
              </button>
              <button class="ignore-button" @click="ignoreIssue(issue)">
                Ignore
              </button>
            </div>
          </div>
          ...
          <div
            v-if="consistencyIssues.length === 0 && hasRunCheck"
            class="no-issues"
          >
    ...
    <div
      ...
      @click="showCharacterDialog = false"
    >
      <div class="character-dialog" @click.stop>
        <div class="dialog-header">
          ...
          <button class="close-button" @click="showCharacterDialog = false">
        ...
        <div class="dialog-content">
          ...
          <div class="form-group">
            <label>Physical Description</label>
            <textarea
              v-model="characterForm.physical_description"
              rows="3"
              class="form-textarea"
            ></textarea>
          </div>
        ...
        <div class="dialog-actions">
          ...
          <button class="dialog-button primary" @click="saveCharacter">
            Save
          </button>
        </div>
      </div>
  ...
  setup(props) {
    ...
    const filteredCharacters = computed(() => {
      ...
      if (characterFilter.value) {
        filtered = filtered.filter(
          (char) => char.type === characterFilter.value,
        );
      }
    ...
    const loadWorldContent = async () => {
      try {
        const response = await fetch(
          `/api/anime/projects/${props.projectId}/bible/world`,
        );
    ...
    const deleteCharacter = async (character) => {
      ...
      try {
        await fetch(
          `/api/anime/projects/${props.projectId}/bible/characters/${character.id}`,
          {
            method: "DELETE",
          },
        );
    ...
    const saveCharacterDetails = async () => {
      try {
        await fetch(
          `/api/anime/projects/${props.projectId}/bible/characters/${selectedCharacter.value.id}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
    ...
    const runConsistencyCheck = async () => {
      ...
      try {
        const response = await fetch(
          `/api/anime/projects/${props.projectId}/bible/consistency-check`,
          {
            method: "POST",
          },
        );
    ...
    const generateCharacterVariant = async (character) => {
      try {
        const response = await fetch(
          `/api/anime/projects/${props.projectId}/bible/characters/${character.id}/generate-variant`,
          {
            method: "POST",
          },
        );
    ...
    const insertTemplate = () => {
      ...
      const template =
        templates[activeWorldCategory.value] || templates.setting;
    ...
    const applyFix = async (issue) => {
      try {
        await fetch(
          `/api/anime/projects/${props.projectId}/bible/consistency-fix`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    const ignoreIssue = (issue) => {
      consistencyIssues.value = consistencyIssues.value.filter(
        (i) => i.id !== issue.id,
      );
    };
