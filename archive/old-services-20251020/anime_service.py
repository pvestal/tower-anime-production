    ...
    try:
        ...
        for char in project.characters:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (project_id, char.name, char.description, char.design_prompt, char.voice_id))
        
        # Add scenes
        for i, scene in enumerate(project.scenes):
            workflow = generate_comfyui_workflow(scene.description, project.style)
            cursor.execute('''
                INSERT INTO scenes (project_id, scene_number, description, duration_seconds, workflow_json)
                VALUES (?, ?, ?, ?, ?)
            ''', (project_id, i+1, scene.description, scene.duration_seconds, json.dumps(workflow)))
...
async def wait_for_comfyui_completion(client, prompt_id: str, project_id: int, scene_id: int, max_wait: int = 300):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not still_processing:
                    ...
                    output_response = await client.get(f"{COMFYUI_URL}/api/history/{prompt_id}")
...
async def create_video_from_frames(frame_paths: list, output_path: str):
    ...
    try:
        ...
        cmd = [
            ...
            '-vf', 'scale=768:512',
            '-c:v', 'libx264',
            '-r', '1',  # 1 FPS since each frame should show for 3 seconds
            '-pix_fmt', 'yuv420p',
            output_path
        ]
...
async def generate_anime_task(project_id: int, job_id: int):
    ...
    try:
        ...
        for scene in scenes:
            ...
            # Call ComfyUI with proper API endpoint
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if prompt_id:
                        # Poll for completion
                        frame_path = await wait_for_comfyui_completion(client, prompt_id, project_id, scene_id)
                        ...
                        if frame_path:
                            ...
                            raise Exception(f"ComfyUI generation failed for scene {scene_id}")
                    else:
        ...
        if all_frames:
            ...
            video_created = await create_video_from_frames(all_frames, video_path)
