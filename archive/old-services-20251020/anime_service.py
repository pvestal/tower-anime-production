        ...
        for scene in scenes:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if prompt_id:
                        # Poll for completion
                        frame_path = await wait_for_comfyui_completion(
                            client, prompt_id, project_id, scene_id
                        )
                        ...
                        if frame_path:
                            ...
                            raise Exception(
                                f"ComfyUI generation failed for scene {scene_id}"
                            )
                    else:
                        raise Exception("ComfyUI did not return prompt_id")
                else:
                    raise Exception(
                        f"ComfyUI request failed: {response.status_code} - {response.text}"
                    )
...
def generate_comfyui_workflow(description: str, style: str):
    ...
    return {
        ...
        "9": {
            ...
            "inputs": {"images": ["8", 0], "filename_prefix": "anime_scene"},
        },
    }
