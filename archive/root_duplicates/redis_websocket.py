...
class WebSocketProgressManager:
    ...
    async def register_connection(self, websocket: WebSocketServerProtocol):
        ...
        logger.info(
            f"🔗 WebSocket connected: {websocket.remote_address} (total: {len(self.connections)})"
        )
    ...
    async def unregister_connection(self, websocket: WebSocketServerProtocol):
        ...
        logger.info(
            f"🔌 WebSocket disconnected: {websocket.remote_address} (total: {len(self.connections)})"
        )
    ...
    async def unsubscribe_from_job(
        self, websocket: WebSocketServerProtocol, job_id: str
    ):
    ...
    async def handle_websocket_message(
        ...
        try:
            ...
            if message_type == "subscribe_job":
                ...
                await self.send_to_connection(
                    websocket, {"type": "queue_stats", **stats}
                )
                ...
                for job_id in monitored_jobs:
                    try:
                        ...
                        if status:
                            ...
                            await self.broadcast_to_job_subscribers(
                                job_id,
                                {
                        ...
                        logger.error(
                            f"Error checking job {job_id[:8]} status: {e}")
...
async def start_websocket_server(host: str = "localhost", port: int = 8329):
    ...
    monitoring_task = asyncio.create_task(
        progress_manager.start_progress_monitoring())
...
async def fastapi_websocket_endpoint(websocket: WebSocket):
    ...
    class FastAPIWebSocketAdapter:
        def __init__(self, ws):
            self.ws = ws
            self.remote_address = (
                (ws.client.host, ws.client.port) if ws.client else ("unknown", 0)
            )
